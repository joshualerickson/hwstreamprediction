...
run some fake data and test.
```{r}
faked <- traintune[c(1:50,6000:6050),c(1,5:21)]
levels(faked$stream) <- c("X0","X1")
set.seed(949)
library(randomForest)
mod1 <- train(stream ~ ., data = faked,
              method = thresh_code,
              metric = "J",
              maximize = FALSE,
              tuneGrid = expand.grid(mtry = c(2,4,6), ntree = c(1000,2000),
                                     threshold = seq(0.1,.99, .05)),
...
mod1
```
It works. What's nice is that it selects the best model at a certain threshold with the J statistic and then can use that for predicting/reporting.
