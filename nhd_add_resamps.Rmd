...
Ktune80_topo$bestTune
Ktune40_topo$bestTune
KtunenoSD_topo$bestTune
```
...
resamples.tune80_topo <-  Ktune80_topo$pred %>% filter(mtry == 2, ntree == 800, threshold == 0.5)
resamples.tune40_topo <-  Ktune40_topo$pred %>% filter(mtry == 2, ntree == 400, threshold == 0.48)
resamples.tunenoSD_topo <-  KtunenoSD_topo$pred %>% filter(mtry == 2, ntree == 800, threshold == 0.46)
...
colnames(resamples.tune80_topo)[2] <- "X"

#now join for the CV80
join_resamp80 <- left_join(resamples.tune80,train,  by = "X" )

join_resamp80 <- join_resamp80 %>% rename_at(vars(obs, pred, X1, X2, mtry, ntree, threshold, Resample),funs(paste(., "topoclimatic", sep = "_")))

join_resamp80_topo <- resamples.tune80_topo %>%  rename_at(vars(obs, pred, X1, X2, mtry, ntree, threshold, Resample),funs(paste(., "topo_only", sep = "_")))

join_all80 <- left_join(join_resamp80, join_resamp80_topo, by = "X")
####  do for the others ###

#now for the LLOCV
colnames(resamples.tune40)[2] <- "X"
colnames(resamples.tune40_topo)[2] <- "X"

#now join
join_resamp40 <- left_join(resamples.tune40,train, by = "X" )

join_resamp40 <- join_resamp40 %>%  rename_at(vars(obs, pred, X1, X2, mtry, ntree, threshold, Resample),funs(paste(., "topoclimatic", sep = "_")))

join_resamp40_topo <- resamples.tune40_topo %>% rename_at(vars(obs, pred, X1, X2, mtry, ntree, threshold, Resample),funs(paste(., "topo_only", sep = "_")))

join_all40 <- left_join(join_resamp40, join_resamp40_topo,train,    by = "X" )

#now the no spatial dependance "noSD"
colnames(resamples.tunenoSD)[2] <- "X"
colnames(resamples.tunenoSD_topo)[2] <- "X"

#now join
join_resampnoSD <- left_join(resamples.tunenoSD,train,  by = "X" )

join_resampnoSD <- join_resampnoSD %>% rename_at(vars(obs, pred, X1, X2, mtry, ntree, threshold, Resample), funs(paste(., "topoclimatic", sep = "_")))

join_resampnoSD_topo <- resamples.tunenoSD_topo %>% rename_at(vars(obs, pred, X1, X2, mtry, ntree, threshold, Resample),funs(paste(., "topo_only", sep = "_")))

join_allnoSD <- left_join(join_resampnoSD, join_resampnoSD_topo,  by = "X" )
...
```{r}
nhdtune80 <- join_resamp80 %>% select(Resample_topoclimatic, obs_topoclimatic, pred_topoclimatic, X1_topoclimatic, NHDPlus)
nhdtune40 <- join_resamp40 %>% select(Resample_topoclimatic, obs_topoclimatic, pred_topoclimatic, X1_topoclimatic, NHDPlus)
nhdtunenoSD <- join_resampnoSD %>% select(Resample_topoclimatic, obs_topoclimatic, pred_topoclimatic, X1_topoclimatic, NHDPlus)
...
{
    rocObject <- pROC::roc(as.numeric(data$obs_topoclimatic), as.numeric(data$NHDPlus))
    
    rocAUC <- rocObject$auc
    
    out <- c(rocAUC, sensitivity(data[, "NHDPlus"], data[, "obs_topoclimatic"], 
        lev[1]), specificity(data[, "NHDPlus"], data[, "obs_topoclimatic"], lev[2]))
    names(out) <- c("ROC", "Sens", "Spec")
    out
}
fourStatsNhdtune <-  function(dat) {
  out <- c(twoClassSummaryNHD(dat, lev = levels(dat$obs_topoclimatic), model = NULL))
  tab <- caret::confusionMatrix(dat$NHDPlus, dat$obs_topoclimatic,
                                positive = levels(dat$obs_topoclimatic)[1])
...
blah <- nhdtune80 %>% base::split(.$Resample_topoclimatic) %>% map(fourStatsNhdtune)
...
Now do the same for the 40 cluster.


```{r}


blah2 <- nhdtune40 %>% base::split(.$Resample_topoclimatic) %>% map(fourStatsNhdtune)
...
blah3 <- nhdtunenoSD %>% base::split(.$Resample_topoclimatic) %>% map(fourStatsNhdtune)
