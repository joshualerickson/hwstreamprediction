...
resamples.tune40 <-  Ktune40$pred %>% filter(mtry == 2, ntree == 1200, threshold == 0.46)
resamples.tunenoSD <-  KtunenoSD80$pred %>% filter(mtry == 3, ntree == 600, threshold == 0.5)
...
nhdtune40 <- join_resamp40 %>% select(Resample, obs, pred, X0, NHDPlus)
nhdtunenoSD <- join_resampnoSD80 %>% select(Resample, obs, pred, X0, NHDPlus)
```
...
blah2 <- nhdtune40 %>% base::split(.$Resample) %>% map(fourStats40tune)
