...
Now we'll need to bring in the NHDPlus layer and then rasterize so that we can get NHDPlus baseline data.
...
Now add that NHDPlus raster to the main raster stack and perform the masking out of wetlands and FSland
...
data34 %>% mutate(accum30 = log(accum30)) %>% select(npol30agg, accum30) %>% gather(key, value) %>% ggplot() + geom_boxplot(aes(value)) + coord_flip() + facet_wrap(~key, scales = "free_y")
...
Mycluster40 <- kmeans(data34[,c(20,21)], (nrow(data34)/40)) 
