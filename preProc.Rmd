...
top34.opt.rad <- data.frame(cbind(top34.opt.rad,top34coords))
...
data34 %>% select(Blue_med, Red_med, Green_med, NIR_med)  %>% map(function(v) {
   uniqv <- unique(v)
...
library(corrplot)
library(caret)
correlations34 <- cor(data34[,-c(1,16,14,9,19,20,21)]) #remove response,coords and HUCs
...
```{r, eval=FALSE}
library(sf)
ptsSF34 <- st_as_sf(data34, coords = c("lon","lat"))
...
Mycluster34 <- kmeans(data34[,c(17,18)], (nrow(data34)/20)) 
